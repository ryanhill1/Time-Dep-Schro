/* ---------------- arrayt.hpp -----------------------
  ...
  if a1 and a2 are arrays (1, or 2 dimensions) and c is a single
  constant of type T type T (usually float or double),
  ...
  a1 = c    : a1 initialized to value c (must be the same type)
  ...
  this source code is formatted for a TAB size of 4 characters
   ...
   remove register int i in operator+=() and 
        add operator=(const T value)  15-nov-2018 ejk
*/
...
// -------  member function operator = single value
template < class T >
...
inline T& arrayt<T>::operator()( const int i1, const int i2 )
{
    ...
    return *(p + i2 + i1*nn2);       //  C ordering
//  return *(p + i1 + i2*nn1);       //  FORTRAN ordering
}
